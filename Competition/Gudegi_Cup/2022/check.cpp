...
int main(int argc, char *argv[])
{
    ...
    {
        if (t == "Yes")
            quitf(_ok, "passed (no counterexample)");
        else
        {
    ...
    else if (s == "No")
    {
        ...
        if (t == "Yes")
            quitf(_fail, "user found counterexample, but judge did not");
        else
        {
            g = ans.readLongs(n, 0LL, 3'000'000'000'000'000'000LL, "g_judge");
            long long i = f(_fail, "judge output is not a counterexample");
            if (h > i)
                quitf(_wa, "judge found a smaller counterexample");
            else if (h < i)
                quitf(_fail, "user found a smaller counterexample");
            else
                quitf(_ok, "passed (valid counterexample)");
        }
    }
}